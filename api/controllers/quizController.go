...
func (quizController *QuizController) Get(ctx context.Context, req *web.Request) web.Result {
	if req.Params["id"] == "" {
		return funcs.CorsedDataResponse(quizController.responder.Data(definitions.GenericAPIMessage{
			Status:  400,
	...
	return funcs.CorsedDataResponse(quizController.responder.Data(GetQuizResponse{
		Status: 200,
		Data:   quiz,
	}))
}
...
func (quizController *QuizController) Update(ctx context.Context, req *web.Request) web.Result {
	...
	intID, err := strconv.Atoi(req.Params["id"])
	if err != nil {
		return funcs.CorsedResponse(quizController.responder.HTTP(500, strings.NewReader(err.Error())))
	}
	//PrintError(err)
	...
	if formError != nil {
		return funcs.CorsedResponse(quizController.responder.HTTP(400, strings.NewReader(formError.Error())))
	}
	...
	if validateError != nil {
		...
		if packError != nil {
			return funcs.CorsedResponse(quizController.responder.HTTP(500, strings.NewReader(packError.Error())))
		}
		return funcs.CorsedResponse(quizController.responder.HTTP(400, strings.NewReader(errorResponse)))
	}
	...
	if err != nil {
		...
		return funcs.CorsedResponse(quizController.responder.HTTP(500, strings.NewReader(errorResponse)))
	}

	res, resErr := json.Marshal(result)
	if resErr != nil {
		return funcs.CorsedResponse(quizController.responder.HTTP(400, strings.NewReader(resErr.Error())))
	}

	return funcs.CorsedResponse(quizController.responder.HTTP(uint(result.Status), strings.NewReader(string(res))))
}
